...
class CommunitiesChooser extends Widget
{
  ...
  protected function renderItems($output = "")
  {
    ...
    foreach ($this->getMemberships() as $membership) {
      ...
      // Check if space has child_id in communities table
      if (!$this->inCommunities($membership->space->guid)) {
  ...
  /**
   * Checks if a result is already in $output
   *
   * @param $output
   * @param $result
   * @return bool
   */
  protected function inCommunities($space_id)
  {
    $entries = Community::findAll(["child_id" => $space_id]);
    if (!empty($entries)) {
      return true;
    }
    return false;
  ...
  protected function getFollowSpaces()
  {
    if (!Yii::$app->user->isGuest) {
      return Follow::getFollowedSpacesQuery(
  ...
  protected function canCreateSpace()
  {
    ...
    return $manager->can(new CreatePublicSpace()) ||
      $manager->can(new CreatePrivateSpace());
  }
   ...
   * @throws \Exception
   */
  public static function getSpaceResult(
    $space,
    $withChooserItem = true,
    $itemOptions = []
  ) {
    $spaceInfo = [
      "guid" => $space->guid,
    ...
    if ($withChooserItem) {
      $options = array_merge(
        ["space" => $space, "isMember" => false, "isFollowing" => false],
        $itemOptions
      );
